...
using KnowledgeBank.Web.Controllers;
	...
	public class AccountController : Controller
	{
		...
		public async Task<IActionResult> Login(LoginInputModel model)
		{
			if (ModelState.IsValid)
			{
				...
				{
					_logger.LogWarning(3, "User email not confirmed");
					ModelState.AddModelError(string.Empty, "User Email not confirmed.");
					return View(await _account.BuildLoginViewModelAsync(model));
				}
		...
		public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl = null)
		{
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					await _userManager.AddToRoleAsync(user, Role.ReadOnlyUser);
